...
public void Database_SaveClientData(int client)
{
  if(g_hDatabase != null)
  {
    char szSteamId[64];
    GetClientAuthId(client, AuthId_SteamID64, szSteamId, sizeof(szSteamId));
    for(int iWeaponNum = 0; iWeaponNum < arModifiedWeapons[client].Length; iWeaponNum++)
    {
      if(arModifiedWeapons[client].Get(iWeaponNum) > 0)
      {
        int iDefIndex = CSGOItems_GetWeaponDefIndexByWeaponNum(iWeaponNum);
        int iSkinDef = arStoredWeaponsPaint[client].Get(iWeaponNum);
        ...
        int iStatTrack = arStoredWeaponsStatTrackKills[client].Get(iWeaponNum);
        int iStatTrack_Enabled = arStoredWeaponsStatTrackEnabled[client].Get(iWeaponNum);
        char szBuffer[2048];
        char szNameTag[1024];
    ...
    delete arStoredWeaponsPaint[client];
    delete arStoredWeaponsQuality[client];
    delete arStoredWeaponsWear[client];
    delete arStoredWeaponsPattern[client];
    delete arModifiedWeapons[client];
    delete arStoredWeaponsNametag[client];
    delete arStoredWeaponsStatTrackEnabled[client];
    delete arStoredWeaponsStatTrackKills[client];
  }
}
        ...
        while(results.FetchRow())
        {
          ...
          if(iDefIndex > 0)
          {
            ...
            else
            {
              ...
              arStoredWeaponsQuality[client].Set(iWeaponNum, iQuality);
              arStoredWeaponsPattern[client].Set(iWeaponNum, iPattern);
              arStoredWeaponsNametag[client].SetString(iWeaponNum, szNameTag);
              arStoredWeaponsWear[client].Set(iWeaponNum, fWear);
              arStoredWeaponsStatTrackEnabled[client].Set(iWeaponNum, iStatTrack_Enabled);
              arStoredWeaponsStatTrackKills[client].Set(iWeaponNum, iStatTrack);
              arStoredWeaponsPaint[client].Set(iWeaponNum, iSkinDefIndex);
